...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        f1_score_list = []
        alldup=0
        for num in range(len(test_graphs)//20):
            # print(num)
            sim_list = {}
            labels = {}
            dupinrepo = 0
            for i in range(20):
                batch = test_graphs[i + num * 20]
                # print(batch)
                sim, label = sess.run(
                    [tensors["metrics"]["training"]["sim"], tensors["metrics"]["training"]["label"]], feed_dict=fill_feed_dict(placeholders, batch)
                )
                # print((-sim[0])**0.5)
                # print("sim", 1/(1-sim[0]))
                # sim = np.mean(sim)
                ...
                if label == 1:
                    dupinrepo = dupinrepo + 1

                labels[i] = label
                # print("label:", label, "sim_list[i]:", sim_list[i])
                # 判断sim是否为正
            if dupinrepo == 0:
                continue
            alldup = alldup+dupinrepo
            result = [(x, y) for x, y in sorted(sim_list.items(), key=lambda x: x[1], reverse=True)][:topK]
            # print(result)
            dups = 0
            for x, y in result:
                # if y >= bor and labels[x] == 1:
                #     dups = dups + 1
                if labels[x] == 1:
                    dups = dups + 1
            print("dups", dups)
            print("topk", topK)
            print("dupinrepo", dupinrepo)

            precision = dups / topK
            recall = dups / dupinrepo

            pre_list.append(precision)
            recall_list.append(recall)

        print(alldup)
        print("np.mean(pre_list)", np.mean(pre_list))
        print("np.mean(recall)", np.mean(recall_list))
        # print("np.mean(f1_score)", np.mean(f1_score_list))
